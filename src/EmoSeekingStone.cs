...
namespace OutwardModTemplate
{
    public class EmoSeekingStone : MonoBehaviour
    {
        ...
        {
            TargetVFXPrefab = OutwardHelpers.GetFromAssetBundle<GameObject>("EmosUniques-SeekingStone", "emoseekingstone", "RuneStoneTarget");
        ...
        public virtual void Update()
        {
            ...
            if (CanRun())
            {
                if (CustomKeybindings.GetKey(EmosUniques_SeekingStone.EXTRA_ACTION_KEY_MOD) && CustomKeybindings.GetKeyDown(EmosUniques_SeekingStone.EXTRA_ACTION_KEY) && StoneMode == EmoSeekingStoneMode.PROXIMITY)
                {
        ...
        private void DoGPSModeDetection()
        {
            ...
            if (CurrentRoute != null)
            {
                ...
                if (distanceFromClosestPointInRoute <= DistanceThreshold)
                {
                    ...
                    EmosUniques_SeekingStone.Log.LogMessage($"Seeking Stone : Close to {CurrentRouteIndex} Route Position");
                    ...
                    {
                        CurrentRouteIndex++;
                        EmosUniques_SeekingStone.Log.LogMessage($"Seeking Stone : Moving to Next Way Point {CurrentRoute.RouteName}");
                    }
                    else
                    {
                        EmosUniques_SeekingStone.Log.LogMessage($"Seeking Stone : Reached end of {CurrentRoute.RouteName}");
                        CurrentRoute = null;
                        DisableEmission();
                    }
                }
        ...
        public void AddNewRoute(string routeName, List<Vector3> Positions)
        {
            ...
            {
                EmosUniques_SeekingStone.Log.LogMessage($"Seeking Stone : A Route Already Exists with name {routeName}");
            }
        ...
        public void SetCurrentRouteByName(string routeName, bool startAtNearestPoint = false)
        {
            ...
            if (emoGPSData != null)
            {
                ...
                EmosUniques_SeekingStone.Log.LogMessage($"Seeking Stone : Setting new Route {routeName} Index {CurrentRouteIndex}");
            }
        }
